...
import { getCachedTranslation, setCachedTranslation, generateCacheKey } from '@/lib/cache';
import fs from 'fs';
...
const CONFIG = {
    ...
    SUPPORTED_VIDEO_TYPES: ['video/mp4', 'video/webm', 'video/avi', 'video/mov', 'video/quicktime'],
...
async function downloadYoutubeVideo(videoUrl: string): Promise<{ path: string; mimeType: string; displayName: string }> {
    try {
...
async function downloadDirectVideo(videoUrl: string): Promise<{ path: string; mimeType: string; displayName: string }> {
    try {
...
async function waitForFileProcessing(fileName: string): Promise<void> {
    ...
    while (attempts < CONFIG.MAX_POLL_ATTEMPTS) {
        ...
        console.log(`Processing attempt ${attempts + 1}/${CONFIG.MAX_POLL_ATTEMPTS}, state: ${fileRecord.state}`);
        ...
        if (fileRecord.state === 'FAILED') {
            throw new VideoProcessingError('Video processing failed on Gemini servers');
        }
    ...
    throw new VideoProcessingError('Video processing timeout - file took too long to process');
}
